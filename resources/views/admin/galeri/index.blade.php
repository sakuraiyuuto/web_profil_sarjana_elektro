                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table id="tabel_galeri" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                <th>Nomor</th>
                                                <th>Foto</th>
                                        ...
                                        <tbody>
                                            @foreach ($galeris as $galeri)
                                                <tr>
                                                    ...
                                                    <td><img src="{{ url($galeri->nama_foto) }}"
                                                            style="width:300px;height:auto;object-fit:cover"></td>
                                                    <td>{{ $galeri->release_date }}</td>
                                                    @if ($galeri->deleted_at == '')
                                                        @if ($galeri->release_date <= date('Y-m-d'))
                                                            <td>Rilis</td>
                                                        @else
                                                            <td>Belum Rilis</td>
                                                        @endif
                                                    ...
                                                    @if ($galeri->deleted_at == '')
                                                        <td>
                                                            <button href="#formModalEdit" role="button" data-toggle="modal"
                                                                data-id="{{ $galeri->id }}"
                                                                data-nama_foto="{{ url($galeri->nama_foto) }}"
                                                                data-sumber="{{ $galeri->sumber }}"
                                                                data-release_date="{{ $galeri->release_date }}"
                                                    ...
                                                    @else
                                                        <td>
                                                            <form
                                                                action="{{ url('/admin/galeri/' . $galeri->id . '/restore') }}"
                                                                method="post">
                                                                @csrf
                                                            ...
                                                            <form
                                                                action="{{ url('/admin/galeri/' . $galeri->id . '/delete') }}"
                                                                method="post">
                                                                @csrf
                                                                <button type="submit" class="btn btn-danger"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="formModalLabel">Tambah Data Galeri</h5>
                    <button type="button" class="btn btn-danger" data-dismiss="modal" aria-label="Close">X</button>
                </div>
                <form id="formAdd" action="{{ url('/admin/galeri') }}" method="post" enctype="multipart/form-data">
                    @csrf
    ...
    @foreach ($galeris as $galeri)
        ...
        <div class="modal fade" id="formModalEdit" tabindex="-1" aria-labelledby="formModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="formModalLabel">Ubah Data Galeri</h5>
                        <button type="button" class="btn btn-danger" data-dismiss="modal" aria-label="Close">X</button>
                    </div>
                    <form id="formEdit" action="{{ route('galeri.update', $galeri->id) }}" method="POST"
...
@section('script')
    ...
    <script>
        $(function() {
            ...
            }).buttons().container().appendTo('#tabel_galeri_wrapper .col-md-6:eq(0)');
        });
